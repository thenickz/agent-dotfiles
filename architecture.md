...
|---|---|---|
| NODE(A) | agent-dotfiles repo (source of everything) | -> NODE(A1), NODE(A2), NODE(A3), NODE(A4), NODE(A5) |
...
| NODE(A3) | scripts/ (validate.sh) | NODE(A) -> NODE(A3) -> NODE(A2), NODE(A4) |
| NODE(A4) | plugins/ (opencode-memory.js enforcement) | NODE(A) -> NODE(A4) -> NODE(B) |
| NODE(A5) | SETUP.md + templates/ONBOARDING.md (one-shot LLM bootstrap) | NODE(A) -> NODE(A5) -> NODE(C) |
| NODE(B) | Global installation (`~/.claude/skills`, `~/.agents/skills`, `~/.config/opencode/plugins`) | NODE(A2), NODE(A4) -> NODE(B) -> NODE(C) |
...
- `NODE(A4) -> NODE(B) -> NODE(C)` — opencode memory plugin enforces memory.md updates in every project
